...
export const onCall_ACL_Add = functions.https.onCall(async (data, context) => {
  ...
  if (context.auth) {
    ...
    const adminPermission = (await admin.database().ref(`/acl/${resourceId}/admin/${context.auth.uid}`).once('value')).val();
    if (adminPermission) {
      admin.database().ref(`/acl/${resourceId}/${action}/${uid}`).set({
...
export const onCall_ACL_Remove = functions.https.onCall(async (data, context) => {
  ...
  if (context.auth) {
    ...
    const adminPermission = (await admin.database().ref(`/acl/${resourceId}/admin/${context.auth.uid}`).once('value')).val();
    if (adminPermission) {
      admin.database().ref(`/acl/${resourceId}/${action}/${uid}`).remove();
    } else {
      throw new functions.https.HttpsError('permission-denied', 'User does not have permissions for the requested resource.');
    }
